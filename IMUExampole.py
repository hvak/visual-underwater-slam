...
def IMU_example():
    ...
    for i in range(80):
        ...
        if i > 0:
            ...
            nRb = scenario.rotation(t).matrix() #Gets rotation matrix from body to navigational frame
            bRn = np.transpose(nRb) #Converts nRb to be from navigational to body
            measuredAcc = scenario.acceleration_b(t) - np.dot(bRn, n_gravity) #removes measured gravitational accel
            measuredOmega = scenario.omega_b(t) #Gets gryo measurement
